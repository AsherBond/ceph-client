...
struct rbd_device {
	...
	char			image_name[RBD_MAX_OBJ_NAME_LEN];
	int			image_name_len;
	char			header_name[RBD_MAX_HEADER_NAME_LEN];
...
static int rbd_header_set_snap(struct rbd_device *rbd_dev, u64 *size)
{
	...
	BUILD_BUG_ON(sizeof (rbd_dev->snap_name) < sizeof (RBD_SNAP_HEAD_NAME));
			  ...
			  u64 *ver)
{
	...
	layout->fl_pg_pool = cpu_to_le32(rbd_dev->poolid);
			       ...
			       gfp_t gfp_flags)
{
	...
	ret = ceph_monc_create_snapid(monc, rbd_dev->poolid, &new_snapid);
			      ...
			      size_t options_size)
{
	...
	*mon_addrs = buf;
	...
	len = copy_token(&buf, rbd_dev->image_name,
					sizeof (rbd_dev->image_name));
	if (!len || len >= sizeof (rbd_dev->image_name))
		return -EINVAL;
